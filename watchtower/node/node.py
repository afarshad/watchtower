...
class Node():
    ...
    def __init__(self, config_path):
        ...
        Sets up signal handling to deal with interrupts. Loads configuration
        file, checks validity and creates sensible defaults if values missing.
        Starts loaded modules. Initialises logger to handle output durring
        running. Creates and binds socket for inter-process communications with
        server instances. Allocates potential ports for server instances. Starts
        continuous communication  with controller. Starts JSON server ready to
    ...
    def _create_config_defaults(self):
        ...
        # TODO change the configuration file to fit the qoem requirements.
        config = dict()
        ...
        config['log_path'] = '/var/log/qoem/node'
...
class RemoteProcedureCall():
    ...
    def start(node, params):
        """Create new qoem  or restart existing ones.
...
class ControllerCommunication():
    ...
    def _send_hello_to_controller(self):
        ...
        while (self._node.node_id == None):
            try:
                ...
                self._node.print_info(TAG, 'qoem node connected to controller, given ID: %s' % self._node.node_id)
            except Exception:
    ...
    def _send_keep_alive_to_controller(self):
        ...
        if self._reset == 1:
            ...
            self._reset = 0
            self._node.print_warn(TAG, 'Connection to qoem controller restarted')
            self._send_hello_to_controller()
            return
        else:
            threading.Timer(interval=int(self._node.config["keep_alive_interval"]), function=self._send_keep_alive_to_controller, args=()).start()
